save_dir = "/home/user/SpectroCCD/Images/2016-06-17/";
...
run("Memory & Threads...", "maximum=2332 parallel=2 run"); 

// if we want a separate thread with a window for start/resume/stop.... use doCommand
// and multithreads...
// doCommand("My Plugin");
...
for (i = 0; i<num; i++) {
	...
	if ( do_unshuffle ) {
		...
		if (rotation_angle != 0.) {
			run("Select All");
			run("Rotate... ", "angle="+(rotation_angle*pixel_length/pixel_width)+" grid=1 interpolation=None");	
	}
