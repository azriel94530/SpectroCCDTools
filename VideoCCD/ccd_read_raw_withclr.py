...
if(len(sys.argv) != 3):
  print "Usage: python Default_Directory Exposure_time_minutes"
  exit()
 
# 
DirectoryPath = sys.argv[1]

Exposure_time_minutes = sys.argv[2]
...
print "Clear Image"
response = urllib2.urlopen(controller_ip + "cmd/ccd_clear", params)
time.sleep(5.0 + float(Exposure_time_minutes)*60)
# Get all data
html = response.read()
...
# Armin has a bit that I should be checking instead
	...
	if ( payload_dict['State'] == '621580' or payload_dict['State'] == '97292' ) :
		not_done = False
