	...
	/* read more data out of the file and send it */
        while (1) {
            ...
            if (sndbuf < BUFSIZE)
                return ERR_OK;
#ifdef DEBUG_OUTPUT
            xil_printf("attempting to read %d bytes, left = %d bytes fd= %d\r\n", BUFSIZE, a->fsize, a->fd);
#endif
			if(a->fd < -999){ //dummy fd for image data transmit. FD contains origin pointer
//	            xil_printf("send buffer %d remaining %d bytes pointer= %x\r\n", sndbuf, a->fsize, a->fd);
				if (BUFSIZE < a->fsize){ //more than one buffer of image data left
            	   tcp_write(tpcb, (char *) a->fd, BUFSIZE, 0); //data is not volatile, send NOCOPY flag
            	   a->fsize -= BUFSIZE; //image data to be sent
            	   a->fd += BUFSIZE; //start pointer moves
               }else{ //last transfer
            	   tcp_write(tpcb, (char *) a->fd, a->fsize, 0); //data is not volatile, send NOCOPY flag
                   a->fd = 0;
                   a->fsize = 0;
                   break;
               }
